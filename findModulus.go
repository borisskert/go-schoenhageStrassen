...
*/
func findModulus(A []int64) (Modulus, Generator, Omega, error) {
	n := int64(len(A))
	...
	if modExp(omega, k, N) != 1 && modExp(omega, n, N) != 1 {
		return -1, -1, -1, fmt.Errorf("invalid ω")
	}

	return Modulus(N), Generator(g), Omega(omega), nil
}

func findModulus2(a, b []int64) (Modulus, Generator, Omega, error) {
	...
	// 5. Define ω≡g^k mod N. We have ω^n=g^(kn)=g^(N−1)=g^(φ(N))≡1 mod N due to Euler’s theorem. Furthermore because g is a
	// generator, we know that ω^i=g^(ik)≢1 for 1≤i<n, because ik<nk=N−1. Hence ω is a primitive nth root of unity, as
	// required by the DFT of length n. // TODO what length ?
	omega := modExp(g, k*n/n, N) // Compute nth root of unity
